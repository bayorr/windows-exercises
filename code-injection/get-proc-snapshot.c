// TODO: each walk functino declares its own HANDLE. To refactor, make a single snapshot and pass that handle to all the functions. Also experiement with making the HANDLE static. Compare the differences to see if the threads change between walk calls.
...
/* from microsoft docs:
	...
	specified process
*/
BOOL walk_process_modules(DWORD process) {
...
int main(int argc, char ** argv) {
	if(argc != 2 || !argv) {
		printf("Expected an argument string\n");
		return -1;
    }
	...
	walk_process_modules(target_pid);
